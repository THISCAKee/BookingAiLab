  ...
  return (
    <nav className="rounded-2xl border border-slate-200 bg-white p-2 shadow-[0_18px_50px_-42px_rgba(11,19,36,0.65)]" aria-label="เมนูผู้ดูแลระบบ">
      <div className="flex items-center justify-between gap-3">
        <Link href="/admin/dashboard" className="flex shrink-0 items-center gap-3 rounded-xl px-2 py-2 text-[#0b1324] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-100">
          ...
          <span className="hidden sm:block">
            <span className="font-display block text-sm font-bold tracking-[0.12em]">BOOKING<span className="text-amber-600">AI</span>LAB</span>
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                ...
                className={`rounded-xl px-4 py-3 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-100 ${active ? "bg-[#0b1324] text-white" : "text-slate-500 hover:bg-slate-50 hover:text-[#0b1324]"}`}
              >
                {item.label}
              </Link>
            );
