...
export default async function AdminLoginPage({
  ...
  return (
    <main className="min-h-screen bg-white px-4 py-5 text-[#0b1324] sm:px-7 sm:py-8">
        ...
        <section className="relative hidden overflow-hidden bg-[#0b1324] p-10 text-white lg:flex lg:flex-col lg:justify-between xl:p-14">
          <div className="absolute inset-x-0 top-0 grid grid-cols-6 gap-px opacity-20" aria-hidden="true">
            {Array.from({ length: 6 }, (_, index) => (
              <span key={index} className={`h-1 ${index < 4 ? "bg-amber-400" : "bg-slate-600"}`} />
            ))}
          </div>

          <div>
            <Link href="/booking" className="inline-flex items-center gap-3 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-400/30">
              <span className="grid h-10 w-10 grid-cols-2 gap-0.5 rounded-xl bg-white p-2.5" aria-hidden="true">
                ...
                <span className="rounded-sm bg-[#facc15]" />
                <span className="rounded-sm bg-[#eab308]" />
                <span className="rounded-sm bg-[#0b1324]" />
              </span>
              <span className="font-display text-sm font-bold tracking-[0.16em]">BOOKING<span className="text-amber-300">AI</span>LAB</span>
            </Link>

            <p className="mt-20 text-xs font-semibold uppercase tracking-[0.22em] text-amber-300">MSU AI LAB · OPERATIONS</p>
          ...
          <div className="grid grid-cols-3 gap-3">
            {["LIVE STATUS", "6 WORKSTATIONS", "15S REFRESH"].map((label, index) => (
              <div key={label} className="border-t border-white/15 pt-4">
                <span className={`mb-3 block h-2 w-2 rounded-full ${index === 0 ? "bg-[#16a34a]" : "bg-amber-400"}`} />
          ...
          <div className="mx-auto w-full max-w-md">
            <div className="flex items-center gap-3 lg:hidden">
              <span className="grid h-9 w-9 grid-cols-2 gap-0.5 rounded-xl bg-[#0b1324] p-2" aria-hidden="true">
                <span className="rounded-sm bg-white" /><span className="rounded-sm bg-[#facc15]" /><span className="rounded-sm bg-[#eab308]" /><span className="rounded-sm bg-white" />
              </span>
              <span className="font-display text-sm font-bold tracking-[0.14em]">BOOKING<span className="text-amber-600">AI</span>LAB</span>
            </div>

            <div className="mt-12 flex items-center gap-3 text-xs font-bold uppercase tracking-[0.2em] text-amber-700 lg:mt-0">
            ...
            <Link href="/booking" className="mt-7 inline-flex text-sm font-semibold text-slate-500 transition hover:text-amber-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-100">
              ← กลับหน้าจองเครื่อง
            </Link>
          </div>
        </section>
